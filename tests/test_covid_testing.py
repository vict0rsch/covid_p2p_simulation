...
if __name__ == "__main__":
    ...
    ExpConfig.load_config(path / "test_configs" / "test_covid_test.yml") # test_covid_test = no intervention
    outfile = None
    n_people = 1000
    simulation_days = 30
    ...
    days = [
        str(datetime.datetime(2020, 2, 28, 0, 0) + datetime.timedelta(days=i)).split()[
            0
        ]
    ...
    for m in tm:
        if m["result"] == "positive":
            positive_tests_per_day[str(m["timestamp"]).split()[0]] += 1
        if m["result"] == "negative":
            negative_tests_per_day[str(m["timestamp"]).split()[0]] += 1
